...
useEffect(() => {
    ...
    const GetUser = async () => {
      try {
        ...
        if (getuserData.decoded_token.role !== "super Admin") {
          navigate("/");
          return;
        }
  ...
  const handleMenuClick = (value, id, name, vid) => {
    // alert(id.key);
    // alert(value);
    // setClickedname(name);
    setUsername(name)
...
//   const handleUSerdelete = async () => {
//     console.log("Deleting user with ID:", userid); // Log the user ID being passed
//     try {
                    ...
                    <div className=" px-4 py-3 ">
                      <Dropdown
                        menu={{
                          items,
                          onClick: (e) =>
                            handleMenuClick(employee._id, e, employee.name, employee.primary_id),
                        }}
                        trigger={["click"]}
                      >
