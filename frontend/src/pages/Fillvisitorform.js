...
const Fillthevisitorform = ({
  ...
  const GetvisitorbyuniqueId = async()=>{
    const response = await axios.get(`http://127.0.0.1:5000/visitor/${visitorid}`)

    console.log("visitordata", response.data)
  ...
  const steps = [
    {
      ...
      content: (
       ...
       <label>
        <input  type="radio" name="gender" checked={formData.gender==="male"} onChange={handleChange} ></input>
        <span className=" ms-1">Male</span>
      </label> 
      <label>
        <input  type="radio" name="gender" checked={formData.gender==="female"}  onChange={handleChange} ></input>
  ...
  return (
    <div>
      ...
      >
        ...
        <div  className={` flex flex-row ${current>0?"justify-between":"justify-end"}`} >{current>0 && 
        (
        <Button  className="  rounded-md py-1 px-4" onClick={prev}>Back</Button>
        )}
        {
          current<steps.length-1 && (
            <Button type="primary" className=" bg-sky-500 rounded-md py-1 px-4" onClick={next}>Next</Button>
          )
        }
        </div>
      </form>
...
export default Fillthevisitorform;
