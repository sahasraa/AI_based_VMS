        ...
        if 'embeddings' in record and record['embeddings']:
            ...
            if embedding.shape == (512,):  # Confirm embedding size
                cached_embeddings[collection_type].append({
                    "embedding": embedding,
                    "mongo_id": str(record["_id"]),  # Convert MongoDB _id to string
                    "name": record.get("name", "Unknown"),  # Safely get name
                    "unique_id": str(record.get("unique_id", "None")),  # Convert unique_id to string
                    "image": record.get("base64_image", None)  # Get base64 image if available
                })
            else:
                print(f"Invalid embedding shape for {record.get('name', 'Unknown')}: {embedding.shape}")
...
def register_employee():
    try:
        ...
        if not face_embeddings:
            return jsonify({"message": "No faces detected in the provided images"}), 203
...
#         # Extract data fields
#         category = data["category"]  # Must be 'visitor'
#         # if category != 'visitor':
...
#         created_by = data["created_by"]  # New field to track creator information
...
#             "embeddings": [{"angle": "unspecified", "embedding": embedding} for embedding in face_embeddings],
#             "base64_image": base64_images[0],  # Store the first base64 image directly in the DB
#             "signature_base64": signature,  # Store signature Base64 data directly in the DB
#             "checkin_time": "",
#             "checkout_time": "",
#             "status": "pending",
#             "created_by": created_by,  # New field added
#             "address":address,
...
def sign_in():
    try:
        ...
        user = db.users.find_one({"email_id": email_id})
        if not user:
...
# def edit_employee(employee_id):
    try:
        ...
        # Validate required fields to edit
        editable_fields = [
            'name', 'email_id', 'phone_number', 'designation'
        ]
...
def get_visitors_by_employee_name(employee_name):
    try:
        # Find all visitors where 'employee_name' matches the parameter
        result = visitors.find({"meet_employee": employee_name})

        # Convert the result to a list and handle ObjectId for JSON serialization
        visitors_list = list(result)
        for visitor in visitors_list:
            visitor['_id'] = str(visitor['_id'])

        if not visitors_list:
            return jsonify({"message": "No visitors found for employee: " + employee_name}), 404
    ...
    try:
        # Query the visitors collection by employee name
        visitors_list = visitors.find({"meet_employee": employee_name})
        
        # Convert the cursor to a list of dictionaries and fix the ObjectId issue
        result = []
        for visitor in visitors_list:
            visitor['_id'] = str(visitor['_id'])  # Convert ObjectId to string for JSON serialization
            result.append(visitor)

        if not result:
            return jsonify({"message": "No visitors found for the employee"}), 404
...
#     visitor = visitors.query.filter_by(unique_id=unique_id).first()
#     if visitor:
#         return jsonify({
#             'unique_id': visitor.unique_id,
#             'name': visitor.name,
...
if __name__ == "__main__":
    # Create threads for Flask, FastAPI, and Video Capture
...
if __name__ == "__main__":
    try:
        ...
        fastapi_thread = Thread(target=run_fastapi)
        video_thread = Thread(target=run_video_capture)
        # change_stream_thread = Thread(target=listen_to_changes, daemon=True)
        ...
        fastapi_thread.join()
        video_thread.join()
        # No join for change_stream_thread since it's a daemon
